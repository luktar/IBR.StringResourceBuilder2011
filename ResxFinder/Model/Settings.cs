...
namespace ResxFinder.Model
{
    ...
    public class Settings : ISettings
    {
        ...
        private static Regex regexIgnore;
        ...
        private List<string> m_IgnoreRegex = new List<string>();
        public List<string> IgnoreRegex
        {
            get { return (m_IgnoreRegex); }
            set { m_IgnoreRegex = value; }
        }
        ...
        {
            if (IgnoreRegex == null || IgnoreRegex.Count == 0) return;

            string pattern = string.Join("|", IgnoreRegex);
            regexIgnore = new Regex(pattern);
        }

        public bool TextMatch(string text)
        {
            if (regexIgnore == null) return false;

            return regexIgnore.Match(text).Success; 
        }
    }
}
